...
class Layout: UICollectionViewLayout {
	...
	override func shouldInvalidateLayout(forBoundsChange newBounds: CGRect) -> Bool {
		if let oldBounds = collectionView?.bounds, oldBounds.size != newBounds.size {
            return true
        }
	...
	override func targetContentOffset(forProposedContentOffset proposedContentOffset: CGPoint) -> CGPoint {
		...
		if let frame = layoutAttributesForItem(at: selectedCellIndexPath as IndexPath)?.frame {
            ...
            if cellBottom > collectionViewBottom {
                finalContentOffset = CGPoint(x: 0, y: collectionViewTop + (cellBottom - collectionViewBottom))
